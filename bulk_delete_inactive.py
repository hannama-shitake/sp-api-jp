...
def delete_listings(inactive_listings: list, seller_id: str, dry_run: bool = False) -> tuple:
    """inactive 出品を一括削除する。(deleted, failed) を返す"""
    ...
    for i, listing in enumerate(inactive_listings):
        ...
        if i % 50 == 0:
            logger.info("[bulk_delete_inactive] 削除中: %d/%d", i, total)

        if dry_run:
            logger.info("[bulk_delete_inactive][DRY-RUN] 削除予定: %s (%s) [%s]", sku, asin, status)
            deleted += 1
            continue
        ...
        try:
            ...
            deleted += 1
            logger.debug("[bulk_delete_inactive] 削除完了: %s (%s)", sku, asin)
        except SellingApiException as e:
            logger.warning("[bulk_delete_inactive] 削除失敗: %s (%s) - %s", sku, asin, e)
            failed += 1
        except Exception as e:
            logger.warning("[bulk_delete_inactive] 予期しないエラー: %s - %s", sku, e)
            failed += 1
...
def main():
    ...
    if args.dry_run:
        ...
        deleted, failed = delete_listings(inactive_listings, seller_id, dry_run=True)
    ...
    deleted, failed = delete_listings(inactive_listings, seller_id, dry_run=False)
